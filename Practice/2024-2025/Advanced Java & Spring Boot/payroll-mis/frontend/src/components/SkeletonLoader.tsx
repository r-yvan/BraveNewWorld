...
export function SkeletonLoader({ count = 3, height = 60, type = 'card' }: SkeletonLoaderProps) {
  if (type === 'table') {
  ...
  return (
    ...
    >
      {[...Array(count)].map((_, i) => (
        <div key={i} className="h-20 bg-slate-700/50 rounded-lg border border-slate-600/30" style={{ height: `${height}px` }} />
      ))}
    </motion.div>
  );
}
